...
export const AdminLogin = TryCatch(async (req, res, next) => {
    ...
    res.status(200).cookie("chattie-admin", token, {
...
export const AdminLogout=TryCatch(async(req,res,next)=>{
    return res.status(200).clearCookie("chattie-admin","", {
        expires:0,
        httpOnly: true
    }).json({
        success:true,
...
export const GetAllChats = TryCatch(async (req, res, next) => {
    ...
    const TransformChat = await Promise.all(chats.map(async ({ groupChat, _id, name, members, creator }) => {
...
export const getAllMessages = TryCatch(async (req, res, next) => {
    const message = await Message.find({}).populate("sender", "name avatar").populate("chat", "groupChat")
    const TransformMessage = message.map(({ content, attachments, _id, sender, createdAt, chat }) => {
        return {
            _id,
            attachments,
            content,
...
export const Dashboard = TryCatch(async (req, res, next) => {
    const [TotalMessages, TotalChats, TotalGroupChats, TotalUsers] = await Promise.all([
    ...
    last7days.forEach(message => {
        const index = Math.floor((today.getTime() - message.createdAt.getTime()) / dayInMillisec)
        message[6 - index]++
    })
